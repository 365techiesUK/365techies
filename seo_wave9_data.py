...
SEO_WAVE9_PAGES = [
 ...
 {'slug': 'virgin-media-email-wont-add-to-new-outlook',
  ...
  'metaDesc': 'The new Outlook will not add your Virgin Media, ntlworld or blueyonder email. Why it '
              'struggles, the settings Virgin publishes, and the fallback.',
  ...
  'lede': 'The new Outlook &mdash; the one with the toggle in the corner &mdash; either spins for a '
          'minute and gives up on your Virgin Media address, adds it and then never fetches a '
          'single message, or announces that something went wrong without saying what. Classic '
          'Outlook had the account for years without complaint. The address still works in a web '
          'browser. This page covers virginmedia.com, ntlworld.com and blueyonder.co.uk addresses '
          'alike, because they are all Virgin Media email now: why the new Outlook struggles with '
          'them, the app password it needs, the settings Virgin itself publishes, and what to do '
          'when Virgin won&rsquo;t give you an app password.',
  'ctaHead': 'Still won&rsquo;t add?',
  'ctaSub': 'We set up email in Outlook and on phones for people across Bournemouth, Poole and '
  ...
  'sections': [{'eyebrow': '/01 &mdash; WHY THE NEW OUTLOOK STRUGGLES',
                ...
                'html': '<p>The new Outlook is not an update to the old one. It is a separate '
                        ...
                        'provider has retired, or leaves the outgoing server without the sign-in '
                        'Virgin requires. The result is the spinning circle, or an account that '
                        'appears but stays empty.</p><p>Two more things about Virgin addresses in '
                        'particular. First, <strong>Outlook cannot sign in with your normal Virgin '
                        'password</strong>. Virgin&rsquo;s own help page says every email program '
                        '&mdash; Outlook, Thunderbird, the mail app on a phone &mdash; needs a '
                        'separate <strong>Virgin Media Mail app password</strong>; the normal '
                        'password only opens webmail. It is the first thing to check when Outlook '
                        'refuses, and section 03 shows where to get it. Second, ntlworld, '
                        'blueyonder and virgin.net addresses are older Virgin brands that now run '
                        'through the same Virgin Media email service, so everything on this page '
                        'applies to them too, app password included.</p>'},
               {'eyebrow': '/02 &mdash; PROVE THE ACCOUNT',
                'h2': 'Two minutes that tell you whether the account or Outlook is at fault',
                'html': '<p>Before touching Outlook, sign in to Virgin Media&rsquo;s webmail with '
                        'the full address and your normal My Virgin Media password. Webmail is the '
                        'one place that still takes the normal password, so it tells you whether '
                        'the account itself is healthy.</p><ul><li><strong>Webmail refuses the '
                        'password.</strong> The account is the problem; no Outlook setting will '
                        'help until it is sorted. Reset the password through My Virgin Media and '
                        'try again. If Virgin has told you the mailbox is suspended, that is '
                        'Virgin&rsquo;s move to Junara, covered in section '
                        '06.</li><li><strong>Webmail opens and the mail is there.</strong> The '
                        'account is fine and the problem is on the Outlook side. That is the usual '
                        'outcome and the good one. Check first that Outlook is being given the app '
                        'password, not the normal one; section 03 covers it.</li></ul><p>While you '
                        'are in webmail, check the newest message&rsquo;s date. If mail is still '
                        'arriving there, nothing is lost and the job is purely to get Outlook to '
                        'see it.</p>'},
               {'eyebrow': '/03 &mdash; THE APP PASSWORD',
                'h2': 'Get the Virgin app password first',
                'html': '<p>Outlook signs in to your Virgin mailbox with a separate <strong>Virgin '
                        'Media Mail app password</strong>, not the password you use for My Virgin '
                        'Media or webmail. Virgin&rsquo;s help page describes making one in the '
                        '<strong>My Virgin Media app</strong> on your phone: go to <strong>Account '
                        'settings</strong>, then <strong>Account details</strong>; under '
                        '<strong>Virgin Media Mail</strong>, tap <strong>Manage</strong> next to '
                        '&lsquo;Manage your Virgin Media Mail app password&rsquo;, then '
                        '<strong>Generate new App password</strong>. Write it down exactly as '
                        'shown.</p><p>Two things catch people out:</p><ul><li><strong>A new app '
                        'password stops the old one.</strong> Any phone, tablet or other computer '
                        'already reading the mailbox stops until you give it the new password too. '
                        'Note everything that checks this address before you make '
                        'one.</li><li><strong>A second address on the account has to be signed in '
                        'as itself.</strong> Signed in as the main account holder, you only see the '
                        'app password for the main address. For a partner&rsquo;s or a second '
                        'mailbox, sign out of the app and sign back in with <em>that</em> address '
                        'and its own password &mdash; Virgin&rsquo;s help says additional accounts '
                        'are managed that way.</li></ul><p><strong>If the option is simply not '
                        'there,</strong> you are not alone. Since Virgin moved its logins to the '
                        'Virgin Media O2 ID, Virgin&rsquo;s own community forum has filled with '
                        'people who cannot find it, especially for second mailboxes. Using your '
               ...
               {'eyebrow': '/04 &mdash; ADD IT BY HAND',
                ...
                'html': '<p>Remove the failed account from the new Outlook if it half-added, then '
                        'add it again, and this time do not let Outlook guess. In its settings, '
                        'under Accounts, choose to add an account, enter the address, and when it '
                        'fails or offers the choice, take the manual or advanced route and pick '
                        '<strong>IMAP</strong>, not POP.</p><p>At the time of writing, Virgin '
                        'Media&rsquo;s own help pages give these settings: incoming server '
                        '<code>imap.virginmedia.com</code> on port 993 with SSL; outgoing server '
                        '<code>smtp.virginmedia.com</code> on port 465 with SSL; the username is '
                        'your full email address; the password, for incoming and outgoing alike, is '
                        'the <strong>app password</strong> from section 03, not your normal one; '
                        'and the outgoing server requires authentication, using the same address '
                        'and app password. Enter each one yourself rather than accepting what '
                        'Outlook filled in. Providers change these occasionally, so if it still '
               ...
               {'eyebrow': '/06 &mdash; THE BIGGER QUESTION',
                ...
                'html': '<p>Once Outlook is working again, spend five minutes on this, because '
                        'Virgin Media email is changing hands. Virgin is handing its whole email '
                        'service &mdash; virginmedia.com, ntlworld.com, blueyonder.co.uk and '
                        'virgin.net &mdash; to a company called <strong>Junara</strong>. To keep '
                        'your address you sign up with Junara before the date in the email Virgin '
                        'sends you. If you still have Virgin broadband, TV or a landline, the first '
                        '12 months are free (plus a 99p transfer fee); after that, and from the '
                        'start if you have left Virgin, you pay Junara monthly or yearly for each '
                        'mailbox, at a price shown once you start the sign-up. Miss the date and '
                        'the mailbox is suspended; 120 days later it is deleted for good.</p><p>If '
                        'you do sign up, the Outlook setup on this page carries on: Virgin says '
                        'email programs such as Outlook should not need any settings changed after '
                        'the transfer. If you would rather not pay every year, the alternative is '
                        'an address that belongs to you rather than your provider &mdash; Gmail or '
                        'Outlook.com for most people &mdash; with the Virgin address forwarding '
                        'into it while friends, the bank and the shops are moved across at your own '
                        'pace. We have written up <a href="/move-virgin-media-email-to-gmail/">how '
                        'to move Virgin email to Gmail</a>, and we do the whole move for you for '
                        '&pound;60 per email address, agreed before we start.</p><p><strong>Watch '
                        'out for fake &lsquo;Junara&rsquo; emails.</strong> Virgin says its own '
                        'emails contain no sign-up link and never ask for payment details, so do '
                        'not pay through an emailed link or a phone call. If you are not sure, call '
                        'us before you click.</p><p '
                        'style="font-size:.85rem;color:var(--muted)">Source: Virgin Media, <a '
                        'href="https://www.virginmedia.com/help/broadband/manage-email-settings" '
                        'rel="noopener" target="_blank">email server settings</a> and <a '
                        'href="https://www.virginmedia.com/help/email-1/email-service-change" '
                        'rel="noopener" target="_blank">email service change</a>, read 26 September '
                        '2026, and Virgin Media O2&rsquo;s <a '
                        'href="https://news.virginmediao2.co.uk/helping-customers-understand-upcoming-changes-to-their-virgin-media-email-services/" '
                        'rel="noopener" target="_blank">announcement of 9 September 2026</a>.</p>'}],
  'faqs': [{'q': 'What are the Virgin Media email settings for Outlook?',
            'a': 'Virgin&rsquo;s own help page gives: incoming (IMAP) imap.virginmedia.com on port '
                 '993 with SSL; outgoing (SMTP) smtp.virginmedia.com on port 465 with SSL, with '
                 'authentication. The username is your full email address, and the password for '
                 'both is your <strong>Virgin Media Mail app password</strong>, not your normal '
                 'password. Choose IMAP, not POP. If Outlook still refuses, check them against '
                 'Virgin&rsquo;s email settings page, which is the final word.'},
           {'q': 'Why can&rsquo;t I get a Virgin Media app password?',
            'a': 'The usual reasons: you are looking in webmail, which does not have it, rather '
                 'than the My Virgin Media app, which is where Virgin describes making it (Account '
                 'settings, Account details, Virgin Media Mail); or it is a second address on the '
                 'account, which has to be signed in to the app as itself, with its own password. '
           ...
           {'q': 'Do ntlworld and blueyonder addresses use the same settings?',
            'a': 'They are older Virgin brands whose email now runs through Virgin Media&rsquo;s '
                 ...
                 'app password as a virginmedia.com address. If in doubt, Virgin&rsquo;s own '
                 'settings page is the reference.'},
           {'q': 'Can I go back to the old Outlook?',
            'a': 'Yes. The toggle at the top-right of the new Outlook switches back to classic '
                 ...
                 'did. It still needs the Virgin app password, but for a stubborn Virgin account it '
                 'is often the quickest route, and we have a separate guide to the switch.'},
           {'q': 'I have left Virgin Media &mdash; is my email gone?',
            'a': 'Not necessarily. Virgin is handing its email service to Junara, and existing '
                 'addresses can be kept by signing up with Junara and paying for them before the '
                 'date in the email Virgin sends you. Miss that date and the mailbox is suspended, '
                 'then deleted 120 days later, and deleted mail cannot be recovered. If you would '
                 'rather not pay, move what you need out first: we can move it all to Gmail for '
                 '&pound;60 per address.'},
           {'q': 'Should I move to Gmail or Outlook.com instead?',
            'a': 'If you would rather not pay Junara every year, or might ever change broadband '
                 'supplier, yes: an address from your provider lives only as long as the account. '
                 'The move can be gradual &mdash; the new address in place, the Virgin one '
                 'forwarding into it &mdash; and we have written up the steps and do it for people '
                 'remotely.'}],
  'chips': ['No fix, no fee', 'Rated 4.9 on Google', 'Trading since 1995'],
  'primaryCta': ['Get it looked at', '/contact/'],
