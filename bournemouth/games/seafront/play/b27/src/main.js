...
// <<< OVERBOARD
// >>> DOLPHINWATCH
import { initWatch } from './dolphinwatch/mode.js';
// <<< DOLPHINWATCH
// >>> SURFER
import { initSurfer } from './surfer/mode.js';
// <<< SURFER
// >>> RAID
...
// >>> ANALYTICS  (src/ui/analytics.js: six anonymous events, live domain only)
import { track, watchLoad, levelFromSearch, startLibrary } from './ui/analytics.js';
// <<< ANALYTICS
// <<< PROGRESS
// <<< STARTPICK
// >>> WIREMUSIC
...
import { PauseMenu, otherSheetOpen, briefingOpen } from './ui/pausemenu.js';
// <<< PAUSEMENU
...
function applySound() {
  ...
  // >>> DOLPHINWATCH
  // 4. The ninth level's blips own a fifth context. Its row id is also the name of the pod's
  // own console handle (window.efoilDolphins, which has no sound), so it is reached by its
  // module's handle rather than through the table walk above.
  const dwm = window.efoilWatch;
  if (dwm && dwm.setSound) dwm.setSound(on);
  // <<< DOLPHINWATCH
}
...
function musicNow() {
  // >>> DOLPHINWATCH
  // THE NINTH ROW, asked FIRST because it is last in the table - the same backwards walk as the
  // loop below, done by hand. It cannot go through wmMod(): its row id is also the name the
  // renderer gave the pod's console handle (window.efoilDolphins), which has no `active`, so the
  // loop would skip it. It takes the RESCUE bed and kind, as the trip back does: no menace,
  // nobody hostile, and the rescue's two variables are this level's own - the clock running
  // down brightens it, and each scare (its `chances`) thins it out.
  { const dw = window.efoilWatch; if (dw && dw.active) return { preset: 'rescue', kind: 'rescue', m: dw }; }
  // <<< DOLPHINWATCH
  // >>> SURFER
  // THE TENTH ROW, asked by hand for the reason the ninth is: its module is reached by its own
  // handle. It takes the RESCUE bed, as the two calm levels above do - nobody hostile, no
  // menace - but a kind of its own, because the variable it wants is its own: quieter while you
  // wait for a wave, full while you ride one.
  { const sf = window.efoilSurfer; if (sf && sf.active) return { preset: 'rescue', kind: 'surfer', m: sf }; }
  // <<< SURFER
...
function musicState(now, live) {
  ...
  if (!g) return {};
  // >>> SURFER  src/surfer/mode.js musicState(): intensity from riding, pulse from waves ridden.
  if (now.kind === 'surfer') return now.m.musicState ? now.m.musicState() : {};
  // <<< SURFER
  if (now.kind === 'raid') {
...
// <<< CRAFT
// >>> ANALYTICS
// Load reported once the textures are in; a machine with no WebGL is reported at once, and the
// library started straight away, because that visitor will never get as far as a texture.
if (seaGL) {
  watchLoad(() => seaGL.status(), () => ({
    level: levelFromSearch(location.search),
    craft: craftHub.kind,
    device: document.body.classList.contains('touch') ? 'touch' : 'desktop',
  }));
} else {
  track('game_error', { reason: 'webgl', device: document.body.classList.contains('touch') ? 'touch' : 'desktop' });
  startLibrary();
}
// <<< ANALYTICS
// >>> JUICE
...
// >>> BRIEFING
// ANY input dismisses a level briefing, and does nothing else that frame. Before this the raid
// card could only be waited out - six to nine seconds with the level already running behind it.
// Capture phase and `keydown`/`pointerdown`/`touchstart` together, so it beats every other
// handler and works the same on a keyboard, a mouse and a thumb.
//
// It returns AFTER dismissing rather than letting the same press through: the press that
// clears a briefing should not also fire a rocket. The card's own buttons are exempt, or a
// player aiming at one would dismiss the card before the click landed.
function dismissBriefing(ev) {
  if (!briefingOpen()) return false;
  if (ev && ev.target && ev.target.closest && ev.target.closest('button, a')) return false;
  // The raid HUD re-decides the card's visibility every frame, so hiding the element is not
  // enough - the flag it reads is what has to move. raid-mode owns the instance as `raid.hud`.
  if (raid && raid.hud) raid.hud.introSkip = true;
  // The smuggling run shows and hides its card by hand, so this one really is a class toggle.
  const sg = document.getElementById('sg-intro');
  if (sg) sg.classList.remove('on');
  document.body.classList.remove('sgcard');
  // >>> DOLPHINWATCH  the ninth level's rules card is a plain class toggle too.
  const dw = document.getElementById('dw-intro');
  if (dw) dw.classList.remove('on');
  // <<< DOLPHINWATCH
  // >>> SURFER  the tenth level's rules card is a plain class toggle as well.
  const sfi = document.getElementById('sf-intro');
  if (sfi) sfi.classList.remove('on');
  // <<< SURFER
  return true;
}
for (const t of ['keydown', 'pointerdown', 'touchstart']) {
  addEventListener(t, (e) => {
    // Marked on the EVENT rather than stopped: these are capture-phase listeners on window, and
    // stopPropagation() there does not reliably keep the game's own window-level handlers from
    // running. A flag the handlers check is explicit and cannot be got wrong by phase order.
    if (dismissBriefing(e)) e.__briefingAte = true;
...
addEventListener('keydown', (e) => {
  ...
  // >>> BRIEFING  the press that cleared a briefing does nothing else.
  if (e.__briefingAte) return;
  // <<< BRIEFING
  switch (e.code) {
...
// >>> PROGRESS
// Paint the player's record onto the level picker every time it opens: a tick on each level
// finished, a ring on the one to play next, and one line saying where they are.
//
// Watched on the SHEET rather than hooked into an opener, because the sheet has at least four
// ways in - the start screen, the LEVELS button, the pause menu's CHOOSE LEVEL and every summary
// card's - and a hook on one of them would leave the other three unpainted.
//
// Rows are marked with CLASSES (styled in index.html) and never by editing their text: the
// suites read these labels and compare them.
//
// No-op under ?cal= / ?clean=1 - the picker is not shown there, and nothing may be added to a
// measured frame.
function paintProgress() {
  if (document.body.classList.contains('clean-render')) return;
  const done = prog.done();
  const nextUp = prog.nextUnfinished();
  for (const b of document.querySelectorAll('#tmode [data-m]')) {
    const id = b.dataset.m;
    b.classList.toggle('done', done.has(id));
    b.classList.toggle('nextup', id === nextUp);
  }
  let line = document.querySelector('#tmode .tally');
  if (!line) {
    line = document.createElement('div');
    line.className = 'tally';
    const h = document.querySelector('#tmode h3');
    if (h) h.after(line);
  }
  const { done: n, total } = prog.progress();
  line.textContent = n === 0
    ? `New here? Start with ${prog.title(nextUp)} — or pick any level you like.`
    : n >= total
      ? `All ${total} levels completed. Nicely done.`
      : `${n} of ${total} levels completed — next up: ${prog.title(nextUp)}`;
}
new MutationObserver(() => {
  // Only on the way IN - painting changes the rows' classes, not the sheet's, so this cannot loop.
  if ($('#tmode').classList.contains('on')) paintProgress();
}).observe($('#tmode'), { attributes: true, attributeFilter: ['class'] });
// <<< PROGRESS
...
$('#tmode').addEventListener('click', (e) => {
  ...
  // >>> ANALYTICS  which row, and from where: 'picker' is the start screen
  track('level_select', { level: id, from: levelFromSearch(location.search) });
  // <<< ANALYTICS
  ...
  if (id === 'free') { toast('free ride \u2014 Poole Bay'); return; }   // the bay's name is Poole Bay (owner, 26 Sep 2026)
  if (id === 'rescue') {
...
// <<< OVERBOARD
// >>> DOLPHINWATCH
// THE NINTH ROW, added 24 Sep 2026 on the owner's instruction, in the same shape as the rows
// above it: a <button data-m> and a SIBLING <span class="d">, appended at run time.
//
// TWO listeners, and the first is on the CAPTURE phase on purpose. The shared handler near the
// top of this file enters most levels itself, and the others enter from their own listeners -
// none of them knows this level exists. So leaving it has to happen BEFORE any of them run:
// exiting afterwards would put the default sea back and restart the sim underneath a level
// that had just set both up. A capture listener on #tmode runs ahead of every bubble listener
// on it, whatever order they were added in.
{
  const body = $('#tmode .body');
  if (body && !body.querySelector('[data-m="dolphins"]')) {
    const b = document.createElement('button');
    b.type = 'button'; b.dataset.m = 'dolphins'; b.textContent = 'DOLPHIN WATCH';
    const d = document.createElement('span');
    d.className = 'd';
    d.textContent = 'A pod is in the bay. Find them, go gently, and get them riding your bow for thirty seconds.';
    body.appendChild(b); body.appendChild(d);
  }
  $('#tmode').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (!b || !b.dataset.m || b.dataset.m === 'close' || b.dataset.m === 'dolphins') return;
    const W = window.efoilWatch;
    if (W && W.active) W.exit();
  }, true);
  $('#tmode').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (!b || b.dataset.m !== 'dolphins') return;
    if (needsReload('dolphins', location.search)) return;   // the handler above is navigating
    const W = window.efoilWatch;
    if (!W || !W.enter) { toast('that level is not loaded in this build'); return; }
    if (W.active) W.again(); else W.enter();
  });
}
// <<< DOLPHINWATCH
// >>> SURFER
// THE TENTH ROW (26 Sep 2026, owner: "carry on with stage 2 and add it to the levels"), in the
// shape of the ninth: a <button data-m> and a sibling <span class="d">, appended at run time,
// with the same two listeners - leaving on the CAPTURE phase so this level is gone before any
// other level's handler sets the world up, and entering on the bubble phase.
{
  const body = $('#tmode .body');
  if (body && !body.querySelector('[data-m="surfer"]')) {
    const b = document.createElement('button');
    b.type = 'button'; b.dataset.m = 'surfer'; b.textContent = 'PIER SURF';
    const d = document.createElement('span');
    d.className = 'd';
    d.textContent = 'The sandbank beside the pier. Paddle out, duck-dive the whitewater and ride three waves.';
    body.appendChild(b); body.appendChild(d);
  }
  $('#tmode').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (!b || !b.dataset.m || b.dataset.m === 'close' || b.dataset.m === 'surfer') return;
    const S = window.efoilSurfer;
    if (S && S.active) S.exit();
  }, true);
  $('#tmode').addEventListener('click', (e) => {
    const b = e.target.closest('button');
    if (!b || b.dataset.m !== 'surfer') return;
    if (needsReload('surfer', location.search)) return;   // the shared handler is navigating
    const S = window.efoilSurfer;
    if (!S || !S.enter) { toast('that level is not loaded in this build'); return; }
    if (S.active) S.again(); else S.enter();
  });
  const help = document.querySelector('#help table');
  if (help && !help.dataset.sf) {
    help.dataset.sf = '1';
    const tr = document.createElement('tr');
    tr.innerHTML = '<td><kbd>Space</kbd></td><td>PIER SURF: duck-dive, pop up or kick out - whichever the moment needs</td>';
    help.appendChild(tr);
  }
}
// <<< SURFER
...
//    "free ride - Poole Bay". That is CANCEL plus the one thing CANCEL cannot say:
...
// >>> LEVELHOLD
// THE LEVEL MENU HOLDS THE GAME (b22, 24 Sep 2026). From the owner's own recording: open the
// level list mid-ride and the craft carried on at the throttle it had - it ran up the beach
// at 40 mph while he was reading the list. The sheet now stops the fixed step the same way a
// briefing card does; the render keeps going for the reason given by the BRIEFING gate.
//
// Two exceptions, both the START SCREEN: the first showing, and the attract demo behind it.
// Nobody is playing yet, and a start screen frozen on one frame looks broken, not paused.
function levelSheetHolds() {
  const s = $('#tmode');
  if (!s || !s.classList.contains('on')) return false;
  if (startPick.showing) return false;
  if (attract && source === attract) return false;
  return true;
}
// <<< LEVELHOLD
...
// >>> SHORTFIT
// 24 Sep 2026. The owner opened the game on his phone and got the level sheet's heading, the
// EA line and LEAVE THE GAME - and NO level buttons at all. Nothing could be chosen.
//
// Why: in #tmode the list (.body) is the ONLY flex item allowed to shrink. The heading, the
// progress line and the footer are all flex: 0 0 auto. PICKFIT above swept phones down to
// about 330 px tall and was right for them; below that nobody had looked. Measured with
// tmp-tr196/pickfit.mjs, footer LEAVE button showing as it was for the owner:
//     280 px tall   8 of 8 levels     240 px   4 of 8     200 px   0 of 8  <- his screen
// A landscape phone with its browser bars up, the Facebook in-app browser, or a keyboard
// open all land there. The b17 progress line made it strictly worse: at 200 px it was the
// difference between 4 of 8 and 0 of 8.
//
// Two rules, and neither changes anything on a screen that already fitted:
//   * .body can never be squeezed below about a row and a half. The half row that peeks out
//     is the scroll hint - PICKFIT's note is right that a scrollbar alone is not a fit.
//   * The SHEET scrolls as a last resort. Where there is room, the list still shrinks first
//     and the heading and footer stay pinned exactly as before; only when that minimum is
//     reached does the whole sheet overflow - and then everything, including the EA licence
//     line, is still reachable rather than clipped away.
// On the shortest screens the progress line goes: the ring on the next level says the same.
//
// Its own element after PICKFIT, appended last so it wins at equal specificity, rather than
// edits inside PICKFIT's string - the suites check that block.
{
  const sfSt = document.createElement('style');
  sfSt.id = 'shortfit';
  sfSt.textContent =
    '#tmode { overflow-y: auto; overscroll-behavior: contain; }'
    + '#tmode .body { min-height: 72px; }'
    // On the shortest screens the list STOPS scrolling on its own and the sheet becomes the
    // one scroller. Two nested scroll areas on a phone means a swipe moves whichever one the
    // finger happens to land in - measured at 400 x 130, lockstep scrolling reached only 4 of
    // the 8 levels. With a single scroller every level is reachable by one ordinary swipe.
    + '@media (max-height: 260px) {'
    + '#tmode .tally { display: none; }'
    + '#tmode { padding-top: 8px; padding-bottom: 8px; }'
    + '#tmode h3 { margin-bottom: 4px; }'
    + '#tmode .body { flex: 0 0 auto; overflow: visible; min-height: 0; }'
    + '}';
  document.head.appendChild(sfSt);
}
// <<< SHORTFIT
...
function tick() {
  ...
  // <<< OVERBOARD
  // >>> DOLPHINWATCH
  // Same place and shape as the two levels above. With the level off, freeTick() is free ride's
  // one-time "Dolphins spotted!" - it reads the pod and says so, once, and never drives anything.
  if (watch && watch.active) watch.tick();
  else if (watch) watch.freeTick(() => !!attract && source === attract);
  // <<< DOLPHINWATCH
  // >>> SURFER  PIER SURF (stage 1, unlisted: ?mode=surfer). Same place and shape as the above.
  if (surfer && surfer.active) surfer.tick();
  // <<< SURFER
  // >>> RAID
...
function frame(nowMs) {
  ...
  // >>> BRIEFING
  // A level briefing card on screen FREEZES the fixed step. Before this, the raid card
  // showed for six to nine seconds with the clock running behind it, so reading the
  // briefing cost you the opening of the level. The RENDER deliberately keeps going -
  // freezing the loop screenshots as an empty canvas, see the note by __calFrozen.
  if (!paused && !window.__calFrozen && !briefingOpen() && !levelSheetHolds()) {
  // <<< BRIEFING
    acc += ft;
    let n = 0;
  ...
  // <<< OVERBOARD
  // >>> DOLPHINWATCH  markers and HUD, with the camera that drew the scene (`rcam`, above).
  if (watch && watch.active) watch.frame(rcam);
  // <<< DOLPHINWATCH
  // >>> SURFER
  if (surfer && surfer.active) surfer.frame(rcam);
  // <<< SURFER
  // >>> RAID
...
{
  const q = new URLSearchParams(location.search);
  // >>> SURFER  the board is the surfing level's alone: ?craft=surfboard anywhere else is ignored.
  const want0 = q.get('craft');
  const want = want0 === 'surfboard' && q.get('mode') !== 'surfer' ? null : want0;
  // <<< SURFER
...
// <<< OVERBOARD
// >>> DOLPHINWATCH
// DOLPHIN WATCH (b22): ?mode=dolphins or the ninth row of the picker. All of it lives in
// src/dolphinwatch/, and it builds no GL: the dolphins are the renderer's own pod. Constructed
// LAST of the levels, so every other level's handle exists when it looks for one to stand down.
// Same ctx shape as the five above it.
...
// <<< DOLPHINWATCH
// >>> SURFER
// PIER SURF (26 Sep 2026, stage 1): ?mode=surfer only - not in the picker until the owner says.
// All of it lives in src/surfer/ and src/boats/surfboard.js; the rider is drawn by the line-up's
// own renderer. Same ctx shape as the levels above.
const surfer = initSurfer({
  sim, seaGL, FIXED_DT, toast, defaultSea: DEFAULT_PRESET, craft: craftHub, live,
  restart: () => restart(true),
  setSea: (name) => setSea(Math.max(0, SEA_NAMES.indexOf(name)), true),
  takeInput: (src) => { attract = null; replaying = null; source = src || live; },
