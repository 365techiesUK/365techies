  ...
  draw(cam, vp, sunDir, exposure, time, shadow) {
    ...
    if (me) {
      if (me.state === 2) this._add('rider', me.x, me.yDraw, me.z, me.heading, me.pitch, me.roll, 0.7, 1, 0);
        ...
        // A wipeout: the board tumbling on its leash beside a swimmer, as TUMBLE draws it.
        const spin = me.stateT * 5.0, ch = Math.cos(me.heading), sh = Math.sin(me.heading);
        this._add('loose', me.x + ch * 0.6, me.yDraw + 0.08, me.z + sh * 0.6, me.heading + spin, 0.5 * Math.sin(spin), 1.2, 0.7, 1, 0);
        this._add('prone', me.x - ch * 0.7, me.yDraw - 0.1, me.z - sh * 0.7, me.heading + spin * 0.4, 0.4, 0.9, 0.7, 0.95, 0.6);
