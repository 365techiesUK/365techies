...
//   * Its own seeded RNG (mulberry32, in ../dolphin-pod.js), so it never draws from the sim's stream.
...
import { DolphinPod, MAX_POD, MAX_PUFF, CRUISE } from '../dolphin-pod.js';
...
// ---------------------------------------------------------------------------------------
// WHAT THE POD DOES lives in ../dolphin-pod.js - where they roam, the breath cycle, and (phase 2)
// how they treat a craft: away from a fast jet ski, onto the bow of a steady boat. It has no GL in
// it so it can be tested in node. This class only draws what it says.
export class Dolphins {
  // stage: the ?dolphins= review handle ('view', 'ride', 'flee'), or null for normal play.
  constructor(gl, { count = 6, seed = 0x0d01f1, busy = false, stage = null } = {}) {
    this.gl = gl;
    this.sim = new DolphinPod({ count, seed, busy, stage });
  ...
  get pod() { return this.sim.pod; }
  get puffs() { return this.sim.puffs; }
  get mood() { return this.sim.mood; }
  get busy() { return this.sim.busy; }
  set busy(on) { this.sim.busy = !!on; }
  summon(x, z, dist, heading) { this.sim.summon(x, z, dist, heading); }
  update(sea, time, dt, pose) { this.sim.update(sea, time, dt, pose); }

  // ---- breath (the particles are the pod's; the buffer to draw them is here) ------------
