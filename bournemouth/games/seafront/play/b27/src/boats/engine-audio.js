...
// >>> SURFER  the surfboard has no engine; its sound is the sea's (surf-audio.js), on this context.
import { buildSurf, updateSurf, surfDebug } from './surf-audio.js';
// <<< SURFER
...
export class EngineAudio {
  ...
  _build() {
    ...
    // >>> SURFER
    if (this.kind === 'surfboard') {
      master.gain.setTargetAtTime(0.55, t, 0.4);
      const sv = buildSurf(ac, master, this.noise);
      sv.master = master;
      this.v = sv;
      return;
    }
  ...
  update(h, P, ft) {
    ...
    // >>> SURFER
    if (v.surf) { updateSurf(ac, v, h, dt, live); return; }
    // <<< SURFER
  ...
  debug() {
    const v = this.v, ac = this.ac;
    // >>> SURFER
    if (v && v.surf) return { enabled: this.enabled, gesture: this.gesture, kind: this.kind, state: ac ? ac.state : 'none', voices: true, ...surfDebug(v) };
    // <<< SURFER
    return {
