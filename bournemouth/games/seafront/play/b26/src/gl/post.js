...
export class PostFX {
  ...
  _apply(ctx, w, h) {
    const gl = this.gl;
    // ⚠️ 24 Sep 2026: THE EIGHT getParameter CALLS THAT WERE HERE ARE GONE, and the comment
    // that justified them was wrong. It read: "All of these are client-side cached by the
    // driver, so there is no round trip." A sampling profile of the raid says otherwise -
    // getParameter was 18.7% of all CPU self time, second only to the sea solver. Under ANGLE
    // on D3D11 a state read is not free, and this pass ran eight of them every frame.
    //
    // Deleting them outright is NOT safe: renderer.draw() opens a frame having set only
    // useProgram and depthMask, so it INHERITS depth test, depth func, cull, blend and the
    // active texture unit from whatever ran last - which is this pass. The restore below is
    // load-bearing.
    //
    // So instead of asking the driver what the state was, this now SETS what the rest of the
    // engine expects. Those values are not a guess: a probe recorded the tuple seen here over
    // 257 frames of raid and 257 of free ride, and it was constant -
    //   DEPTH_TEST on, depthMask true, DEPTH_FUNC LEQUAL, CULL_FACE off, BLEND off, VAO null,
    //   ACTIVE_TEXTURE TEXTURE0 (254/257; the three outliers are start-up frames, before the
    //   first post pass matters, and TEXTURE0 is the GL default anyway).
    // Measured on an RTX 3090 / Threadripper at 1920x1080 on the raid: cpuAvgMs 3.64 -> 2.74
    // and 239 -> 307 fps, with the 25-view corpus bit-identical.
    ...
    // Leave the state the next frame expects. The textures on units 10 and 11 are left bound -
    // nothing else in the tree uses a unit above 4 (grep: activeTexture) - but the
    // ACTIVE_TEXTURE selector itself is put back, because that one is shared.
    //
    // Four of these eight are already in the wanted state, because this pass set them on the
    // way in (DEPTH_TEST on, depthMask true, CULL_FACE off, BLEND off). They are written
    // anyway, and deliberately: the next frame's correctness should be readable here without
    // tracing what the top of this function happens to set. A redundant state write is a few
    // nanoseconds; a state read was costing 18.7% of the frame.
    // The PROGRAM is deliberately NOT restored - every draw binds its own, and renderer.draw()
    // opens the next frame with useProgram(skyProg).
