...
export class RaidSprites {
  ...
  draw(cam, vp, sunDir, exposure, time) {
    ...
    // 24 Sep 2026: was `const prevUnit = gl.getParameter(gl.ACTIVE_TEXTURE)`, restored at the
    // end. That is a per-frame driver state read and they measured expensive (see post.js).
    // The unit is TEXTURE0 every time this runs - probed over 257 frames - so the tail of this
    // function sets TEXTURE0 outright instead of asking. The identical call in the CONSTRUCTOR
    // is left alone on purpose: it runs once, so it costs nothing and reads the true value.
    gl.activeTexture(gl.TEXTURE0 + UNIT);
    gl.bindTexture(gl.TEXTURE_2D, this.tex);
