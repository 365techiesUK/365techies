...
export class WatchHud {
  constructor(mount, { onStart, onAgain, onExit }) {
    ...
    r.innerHTML = `
      ...
      <div id="dw-top" class="dw-panel">
        ...
        <span id="dw-ride"><span class="t">ON THE BOW 0 / 30 s</span><span class="bar"><i></i></span></span>
        <span id="dw-compass"><i></i><span>—</span></span>
      </div>
    ...
    this.el = {
      markers: q('#dw-markers'), timer: q('#dw-timer'), rideT: q('#dw-ride .t'), rideBar: q('#dw-ride .bar i'),
      compass: q('#dw-compass'), arrow: q('#dw-compass i'), cdist: q('#dw-compass span'),
  ...
  update(run, pose, proj, W, H, viewYaw, waterAt) {
    ...
    e.rideT.textContent = `ON THE BOW ${Math.floor(run.rideT)} / ${T.rideGoal} s`;
