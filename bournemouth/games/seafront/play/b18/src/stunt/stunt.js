...
export class StuntHud {
  constructor(mount, { onAgain, onLevels } = {}) {
    ...
    this.el.card.addEventListener('click', (e) => {
      ...
      if (!b) return;
      // >>> PROGRESS
      if (b.dataset.a === 'next') { goNext(b); return; }
      // <<< PROGRESS
  ...
  showOver(e) {
    ...
    this.el.card.innerHTML = '<h3>RUN OVER</h3><table>'
      ...
      // >>> PROGRESS  the card is rebuilt from this string every time, so the button lives in it.
      + '<p><button type="button" data-a="next" hidden>NEXT</button> '
      // <<< PROGRESS
      + '<button type="button" data-a="again">RIDE AGAIN</button> '
      + '<button type="button" data-a="levels">LEVELS</button></p>';
    // >>> PROGRESS  set AFTER the rebuild - the element did not exist a line ago.
    setNext(this.el.card.querySelector('[data-a=next]'), 'stunt', true);
    // <<< PROGRESS
    this.el.card.classList.add('on');
  }
