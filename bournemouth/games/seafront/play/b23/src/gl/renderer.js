...
import { once } from '../ui/analytics.js';
// <<< DOLPHINS
  ...
  draw(sim, cam, fovY, mode = 'chase') {
    ...
    if (this.dolphins) {
      ...
      if (!inRaid) {
        ...
        this.dolphins.update(sim.sea, t, dt, pose);
        this.dolphins.draw(cam, this.vpM, this.sunDir, this.exposure, t, this.shadow);
        // does anyone find them? counted once a visit (src/ui/analytics.js; live site only)
        if (this.dolphins.mood === 'ride') once('dolphins_bowride', { craft: pose.craft });
      } else {
        this._dolT = null;
      }
    }
