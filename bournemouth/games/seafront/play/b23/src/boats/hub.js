...
// <<< BEACHBACK
// >>> CAMCLIP  see _unclip()
const CLIP = { step: 0.4, back: 0.7, min: 1.5, lift: 0.3, capsule: 0.4, easeOut: 2.5 };
// <<< CAMCLIP
...
export class CraftHub {
  ...
  draw(camera, ft) {
    ...
    // <<< CAMDYN
    // >>> CAMCLIP
    if (!cc && camera !== 'side' && camera !== 'fpv') this._unclip(cam, Math.max(1 / 240, ft));
    else if (U) U.f = 1;
    // <<< CAMCLIP
  ...
  _unclip(cam, dt) {
    ...
    if (len > CLIP.min) {
      ...
      for (let s = CLIP.step; s < len + CLIP.step; s += CLIP.step) {
        ...
        if (hit) { f = Math.max(CLIP.min, s - CLIP.back) / len; break; }
      }
    }
