...
export class RescueHud {
  constructor(mount, { onEnter, onAgain, onExit }) {
    ...
    r.innerHTML = `
      ...
      <div id="rq-over" class="rq-panel">
        ...
        <!-- >>> PROGRESS  shown after a finished shift when there is a next level -->
    ...
    this.el = {
      ...
      next: q('#rq-over [data-a=next]'),
    };
    // >>> PROGRESS
    q('#rq-over [data-a=next]').addEventListener('click', (e) => { e.stopPropagation(); goNext(e.currentTarget); });
    // <<< PROGRESS
  ...
  showOver(sum, seed) {
    const e = this.el;
    // >>> PROGRESS
    // Rescue has no fail state - the shift ends on the clock, not on a loss - so reaching this
    // card IS the completion.
    prog.complete('rescue', true);
    setNext(e.next, 'rescue', true);
    // <<< PROGRESS
