...
ok($qq['sr'][$oidT]['st'] === 'sent' && $qq['sr'][$oidT]['resent_as'] === $nidT, 'the original stays sent and names its copy');
ok(!isset($qq['sr']['nobody-sent-selfrun-visit']), 'a self-run with no booked visit is left alone');
ok((isset($qq['q']['9010']['dn']) ? $qq['q']['9010']['dn'] : '') === 'superseded' && (isset($qq['q']['9010']['dn_by']) ? $qq['q']['9010']['dn_by'] : '') === $nidT, "the visit's own job-done email is superseded by the copy");
$dbR = json_decode(file_get_contents($dbT), true);
ok(isset($dbR['customers'][$KEY]['machines'][$MACHINE]['repk'][(string)($TS + 10)]) && $dbR['customers'][$KEY]['machines'][$MACHINE]['repk'][(string)($TS + 10)] === 'service', 'the portal tag is now service');
