      ...
      (function () {
        ...
        function sendCode() {
          ...
          post({ action: 'join', email: S.email, bk_name: S.name, bk_phone: S.phone,
            ...
            busyFlag = false;
            if (r && (r.ok || r.have_code)) {
              try { if (window.gtag && localStorage.getItem('tt_internal') !== '1') gtag('event', 'booking_code_sent', {}); } catch (e) {}
              return codeBox(r && !r.ok && r.have_code);
            }
        ...
        function codeBox(already) {
          ...
          document.getElementById('bkre').onclick = function () {
            ...
            post({ action: 'join', email: S.email, bk_name: S.name, bk_phone: S.phone,
                   bk_what: S.svcName || '', bk_when: whenTxt() }).then(function (r) {
              busyFlag = false; reBtn.disabled = false;
              e2.textContent = (r && (r.ok || r.have_code)) ? 'Sent — give it a moment, and do check your junk folder.'
                : 'We couldn’t send another code just now. If the email address above is wrong, tap “Change my details”. Or call 01202 775566.';
            }, function () { busyFlag = false; reBtn.disabled = false; e2.textContent = 'We couldn’t send another code just now — try again in a moment, or call 01202 775566.'; });
          };
