...
def off_grid():
    ...
    content = "\n".join([
      ...
      <div class="wrap">
        ...
        <style>
        .vlive{max-width:920px;margin:0 auto;background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.09);border-radius:20px;padding:1.4rem 1.4rem 1.2rem;box-shadow:0 12px 44px rgba(0,0,0,.28)}
        .vlive__bar{display:flex;align-items:center;gap:.7rem;font:600 .72rem/1 ui-monospace,monospace;letter-spacing:.06em;text-transform:uppercase;color:#9fb0c3;margin-bottom:1.3rem;flex-wrap:wrap}
        ...
        .vlive__grid{display:grid;grid-template-columns:auto 1fr;gap:1.6rem;align-items:center}
        .vlive__soc{position:relative;width:160px;height:160px;flex:none;margin:0 auto}
        .vlive__ring{width:160px;height:160px;transform:rotate(-90deg)}
        .vlive__ringbg{fill:none;stroke:rgba(255,255,255,.08);stroke-width:9}
        .vlive__ringfg{fill:none;stroke:#39d353;stroke-width:9;stroke-linecap:round;stroke-dasharray:326.7;stroke-dashoffset:326.7;transition:stroke-dashoffset 1.1s ease,stroke .6s}
        .vlive__socinner{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center}
        .vlive__socval{font:800 2.2rem/1 inherit;color:#fff}
        .vlive__socstate{font:600 .66rem/1 ui-monospace,monospace;letter-spacing:.09em;text-transform:uppercase;color:#9fb0c3;margin-top:.45rem}
        .vlive__metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:.65rem}
        .vlive__m{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:.8rem .55rem;text-align:center}
        .vlive__mv{display:block;font:800 1.15rem/1.1 inherit;color:#fff}
        .vlive__ml{display:block;font:600 .6rem/1.2 ui-monospace,monospace;letter-spacing:.05em;text-transform:uppercase;color:#9fb0c3;margin-top:.4rem}
        .vlive__tanks{display:flex;gap:1.1rem;margin-top:1.2rem;flex-wrap:wrap}
        ...
        .vlive__tankfill{height:100%;border-radius:6px;background:linear-gradient(90deg,#2aa9e0,#39d353);transition:width 1.1s ease}
        .vlive__note{font:600 .7rem/1.5 ui-monospace,monospace;color:#9fb0c3;text-align:center;margin:1.2rem 0 0}
        .vlive--sample .vlive__live{color:#e0b341}.vlive--sample .vlive__dot{background:#e0b341;animation:none}
        @media(max-width:560px){.vlive__grid{grid-template-columns:1fr}.vlive__metrics{grid-template-columns:repeat(2,1fr)}}
        </style>
        <div class="vlive" id="vlive" data-reveal>
          ...
          <div class="vlive__grid">
            <div class="vlive__soc">
              <svg viewBox="0 0 160 160" class="vlive__ring" aria-hidden="true"><circle cx="80" cy="80" r="52" class="vlive__ringbg"></circle><circle cx="80" cy="80" r="52" class="vlive__ringfg" data-ring></circle></svg>
              <div class="vlive__socinner"><span class="vlive__socval" data-soc>&mdash;</span><span class="vlive__socstate" data-state>&mdash;</span></div>
            </div>
            <div class="vlive__metrics">
              <div class="vlive__m"><span class="vlive__mv" data-pv>&mdash;</span><span class="vlive__ml">Solar now</span></div>
              ...
              <div class="vlive__m"><span class="vlive__mv" data-ba>&mdash;</span><span class="vlive__ml">Battery amps</span></div>
            </div>
          </div>
    ...
    (function(){
      ...
      var C=326.7;
      function n(v,d){return (v==null||isNaN(v))?"—":Number(v).toFixed(d||0);}
      ...
      function set(sel,txt){var x=el.querySelector(sel); if(x) x.textContent=txt;}
      function render(s){
        ...
        set("[data-soc]", n(s.soc)+"%"); set("[data-state]", stateLabel);
        set("[data-pv]", n(s.pvW)+" W"); set("[data-bw]", ((s.battW>0)?"+":"")+n(s.battW)+" W");
        set("[data-yield]", n(s.yieldToday,2)+" kWh");
        set("[data-ttg]", (s.timeToGo==null||s.timeToGo>240)?"∞":n(s.timeToGo)+" h");
        set("[data-bv]", n(s.battV,2)+" V"); set("[data-ba]", n(s.battA,1)+" A");
        set("[data-updated]", s.sample?"sample reading":("updated "+ago(s.updated)));
        var ring=el.querySelector("[data-ring]");
        if(ring){ var soc=Math.max(0,Math.min(100,s.soc||0)); ring.style.strokeDashoffset=(C*(1-soc/100)).toFixed(1); ring.style.stroke=col; }
        var tanks=el.querySelector("[data-tanks]");
        if(tanks){ tanks.innerHTML=(s.tanks||[]).map(function(t){var lv=Math.max(0,Math.min(100,t.level||0));return '<div class="vlive__tank"><div class="vlive__tankhead"><span>'+t.type+'</span><span>'+lv+'%</span></div><div class="vlive__tankbar"><div class="vlive__tankfill" style="width:'+lv+'%"></div></div></div>';}).join(""); }
        var note=el.querySelector("[data-note]");
        if(note){ note.textContent = s.sample ? "Sample reading — the live feed switches on once our VRM link is connected." : "Live from our 365 Crafter, running clean on Victron solar."; }
