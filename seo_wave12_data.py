...
SEO_WAVE12_PAGES = [
 {'slug': 'ferrari-sd2-laptop-windows-xp',
  ...
  'metaDesc': 'Ferrari SD2/SD3 testers need a 32-bit, XP-era laptop with a real serial port. We image, clone and protect that laptop. We never touch the car.',
  ...
  'lede': 'The tester is the rare, expensive part. The laptop it plugs into is the part that dies &mdash; a 32-bit, XP-era machine with a real serial port, carrying ECU data that is hard to get again. We keep that laptop working, with a tested spare on the shelf, for independent specialists and owners across the UK. We don&rsquo;t touch the car.',
  ...
  'sections': [
   {'eyebrow': '/01 &mdash; WHY AN OLD LAPTOP',
    ...
    'html': '<p>The Ferrari <strong>SD2</strong> &mdash; <em>Sistema Diagnosi 2</em> &mdash; was built for Ferrari in the mid-1990s by Digitek, and it is a three-part kit: a handheld tester, the Windows PC software that drives it, and the cables to the car. The tester talks to the PC over an <strong>RS-232 serial link</strong>. The software is a 32-bit Windows program, and the original manual asked for a PC with a serial port and a parallel port running Windows 95.</p>'
            '<p>That is the whole story of the XP dependence. The people who keep these kits going report that the software installs happily on <strong>32-bit Windows up to and including Windows 7</strong>, and not on anything 64-bit. So the real requirement is three things at once: a 32-bit Windows, a <em>real</em> serial port, and the ECU data files that were installed on that machine over the years. Windows XP-era laptops &mdash; ThinkPads with a docking station, older Dells, Panasonic Toughbooks &mdash; are simply the last mainstream machines that have all three, which is why nearly every working SD2 in the country lives on one.</p>'
            '<p>It matters more than it sounds, because the SD2 is not a tool you plug in once a year. The tester has no memory of its own for the car modules; you load them from the PC each time. The laptop is in use every time the tester is, so when it goes, the tester goes with it.</p>'},
   {'eyebrow': '/02 &mdash; WHAT BREAKS',
    'h2': 'What actually goes wrong &mdash; it is almost never the tester',
    'html': '<p>Testers are robust. The laptops were built in the 2000s and have done twenty years of workshop life. What we see:</p>'
            ...
            '<li><strong>No spare.</strong> Most workshops have one laptop for the kit and nothing behind it. The day it fails is the day a clutch job stalls.</li></ul>'
   ...
   {'eyebrow': '/03 &mdash; THE FAMILY',
    ...
    'html': '<p>The SD2 is one of a family, and the laptop story is much the same across it.</p>'
            '<ul><li><strong>SD3.</strong> The later palmtop tester, again from Digitek, with more memory of its own and a PC package (SD3NET) that can connect over serial, USB or Ethernet. It runs standalone for many jobs but still wants the PC for storing diagnostic data and the detailed work. Its own manual lists <strong>Windows XP (Service Pack 1), Windows 2000 and Windows 98 SE</strong> as the supported systems &mdash; XP being the newest. Same era, same kind of laptop.</li>'
            '<li><strong>SDX.</strong> A reissue by Waycon in Italy that brings the SD1, SD2, SD3 and DEIS applications into one USB interface, covering cars up to the 430 Scuderia; Ferrari&rsquo;s own technical site points buyers to it. The vendor datasheet asks for a PC running <em>Windows XP or later</em>, and its &ldquo;SD2 look&rdquo; mode is the same 1990s application underneath. A newer SDX 2.0 exists &mdash; ask Waycon for its current requirements rather than us.</li>'
            '<li><strong>DEIS-Light.</strong> Ferrari&rsquo;s own dealer-level software for the 599 GTB Fiorano onwards, licensed annually through Ferrari with interfaces from the authorised network. A different world, and not what this page is about.</li>'
            ...
            '<p>The point of the list is the timescale. The cars these testers were built for are twenty-five to thirty years old and will be on the road for decades yet. The laptops behind them are not going away; they just need looking after.</p>'},
   {'eyebrow': '/04 &mdash; WHY IT STAYS IN SERVICE',
    'h2': 'What only the factory tool does &mdash; and why you cannot simply retire it',
    'html': '<p>A generic OBD reader gets you engine fault codes on the later cars. It does not get you the rest, and on the earlier cars it does not even plug in: a 355 has a three-pin diagnostic connector, not the 16-pin socket the cheap readers expect. The Ferrari-specific systems &mdash; the F1 gearbox and clutch set-up, ABS and ASR bleeding, airbag, the adaptive suspension, alarm and instrument cluster &mdash; are what the factory tester is for, and specialists are clear that clutch work on some F1 cars cannot be set up correctly without it.</p>'
            '<p>The honest nuance, because you will read it on the owners&rsquo; forums: the modern multi-brand tools now cover a good deal of the later cars, and some busy specialists use them for most of the day. The same threads then list what still sends them back to the SD2, the SD3 or a Leonardo &mdash; the pre-2003 cars that speak the old ISO protocol, clutch set-up on particular models, Maserati proxi alignment. We do not adjudicate which tool is best; that is your trade, not ours. What we can say is that as long as the old tool is the one you reach for, the laptop that runs it needs to be as dependable as the tester.</p>'
  ...
  'faqs': [
   {'q': 'Can I run the SD2 software on a new laptop?',
    'a': 'Not on a normal new one. The software is 32-bit and needs a real serial port to reach the tester; owners report it installs on 32-bit Windows up to Windows 7 and not on 64-bit systems, and a modern laptop has neither the 32-bit Windows nor the port. The dependable answer is to keep an XP-era machine and clone it, or to talk to us about a virtual machine that we bench-test with your kit first.'},
   ...
   {'q': 'Which laptop do people use for the SD2?',
    'a': 'The kits we see and the owners&rsquo; forums agree: ThinkPads with a docking station, older Dells, and Panasonic Toughbooks of the CF-30 and CF-19 generation &mdash; 32-bit XP-era machines with a real serial port. The exact model matters less than those three things.'},
   ...
   {'q': 'Is the SDX the same as the SD2?',
    'a': 'The SDX is Waycon&rsquo;s reissue that brings the SD1, SD2, SD3 and DEIS applications into one USB interface. Its SD2 mode is the same application underneath, and the vendor asks for a PC running Windows XP or later. A newer SDX 2.0 exists; Waycon is the right place to ask about its current requirements.'},
