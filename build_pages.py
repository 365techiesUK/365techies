      ...
      function start(){if(typeof L==='undefined'){return setTimeout(start,200);}
        ...
        function bestCells(){var cells={};
          cache.forEach(function(p){var f=freshDl(p);if(f==null)return;
            ...
            (cells[k]=cells[k]||{lat:[],lon:[],dl:[],lat_ms:[],net:{}}).dl.push(f);
          ...
          return Object.keys(cells).map(function(k){var c=cells[k];
            ...
            return {lat:median(c.lat),lon:median(c.lon),dl:median(c.dl),ms:median(c.lat_ms),n:c.dl.length,net:net};
          ...
          if(mode==='best'){list.hidden=false;list.innerHTML='';
            ...
            cells.forEach(function(c,i){
              var mk=L.circleMarker([c.lat,c.lon],{radius:Math.min(9+c.n,16),color:spdcol(c.dl),fillColor:spdcol(c.dl),fillOpacity:.8,weight:2})
                .bindPopup('<b>'+Math.round(c.dl)+' Mbps</b> median download &middot; '+c.n+' test'+(c.n>1?'s':'')+(c.ms!=null?('<br>'+Math.round(c.ms)+' ms ping'):'')+(c.net?('<br>'+c.net):'')).addTo(layer);
              ...
              if(i<8){var li=document.createElement('li');var w=wr(c.dl);
                li.innerHTML='<span class="rk">'+(i+1)+'</span><span class="mb">'+Math.round(c.dl)+' Mbps</span><span class="meta">'+(c.ms!=null?(Math.round(c.ms)+' ms &middot; '):'')+(c.net?(c.net+' &middot; '):'')+c.n+' test'+(c.n>1?'s':'')+'</span><span class="wr '+w[1]+'">'+w[0]+'</span>';
