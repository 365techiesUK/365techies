...
{FOOTER}
  ...
  <script src="/js/forms.js?v=2" defer></script>
...
# Shared Victron system builder (ULTIMATE: client-side sizing engine -> full component spec + per-circuit cables/lugs/fusing,
# tools list and golden-rules advice, with retail search links per item) — /victron-system-builder/
# Retail links are SEARCH links (never rot, no prices). AMZN_TAG empty = plain links + "we don't earn" disclosure; set a tag and
# the Associates disclosure + rel=sponsored swap in automatically. Bimble pattern is one constant (standard OpenCart search).
# Cable/fuse figures follow Victron's manuals & Wiring Unlimited practice, stated for short runs (<=3m) at 12V.
VBUILDER_TOOL = r'''    <section class="section" aria-label="Victron system builder" id="vbtool">
      <div class="wrap">
        <div class="section-head">
          ...
          <p class="lede lede--center" data-reveal>Answer four questions and get the <strong>complete build</strong> &mdash; every Victron component, plus the exact cables, lugs and fuses for each circuit, the tools you&rsquo;ll need, and the golden rules that keep it safe. Links to buy every part, or one button to have us fit the lot.</p>
        </div>
      ...
      (function(){
        ...
        var MPPTS=[
          {max:220, name:'Victron SmartSolar MPPT 75/15',  battmm:'6mm&sup2;',  fuse:'20A blade/MIDI', lug:'ferrule (screw terminal)', fq:'20A midi fuse'},
          {max:290, name:'Victron SmartSolar MPPT 100/20', battmm:'6mm&sup2;',  fuse:'25A MIDI',       lug:'ferrule (screw terminal)', fq:'25A midi fuse'},
          {max:440, name:'Victron SmartSolar MPPT 100/30', battmm:'10mm&sup2;', fuse:'40A MIDI',       lug:'ferrule (screw terminal)', fq:'40A midi fuse'},
          {max:700, name:'Victron SmartSolar MPPT 100/50', battmm:'16mm&sup2;', fuse:'60A MIDI',       lug:'ferrule (screw terminal)', fq:'60A midi fuse'},
          {max:1000,name:'Victron SmartSolar MPPT 150/70', battmm:'25mm&sup2;', fuse:'100A MEGA',      lug:'25mm&sup2; lugs at the fuse/busbar end', fq:'100A mega fuse'}
        ];
        var INV={
          small:{name:'Victron Phoenix 12/800',q:'Victron Phoenix inverter 12 800',why:'Clean 230V for laptops, TVs and chargers up to ~650W continuous.',
          ...
          micro:{name:'Victron MultiPlus 12/2000/80',q:'Victron MultiPlus 12 2000',why:'Runs a ~900W microwave with headroom, and doubles as an 80A mains charger on hook-up.',
          ...
          kettle:{name:'Victron MultiPlus 12/3000/120',q:'Victron MultiPlus 12 3000',why:'Kettles and induction hobs pull 2&ndash;3kW &mdash; this handles them, and charges at 120A on hook-up.',
        ...
        function calc(){
          ...
          else if(liAh<=400) items.push({cat:'Battery',name:'2&times; Victron SuperPack 12.8V 200Ah',q:'Victron SuperPack 200Ah lithium',why:'You need ~'+liAh+' Ah of lithium for '+Math.round(ahDay)+' Ah/day with buffer &mdash; paired SuperPacks keep it simple (equal-length pair leads!).'});
          else items.push({cat:'Battery',name:'Victron Smart LiFePO4 bank ('+liAh+' Ah) &mdash; talk to us',q:'Victron Smart LiFePO4 330Ah',why:'At this size you&rsquo;re into external-BMS territory with Class-T fusing &mdash; great systems, but they deserve proper design.'});
          items.push({cat:'Solar panels',name:panels+'&times; 175W rigid panel ('+pvTot+'W total)',q:'175W rigid solar panel',why:'Sized for spring/autumn (~'+pvW+'W needed); summer needs only ~'+pvSummer+'W. UK winter is ~1 sun-hour &mdash; plan on engine or hook-up charging then.'});
          var m=null; for(var i=0;i<MPPTS.length;i++){ if(pvTot<=MPPTS[i].max){ m=MPPTS[i]; break; } }
          if(m) items.push({cat:'Solar controller',name:m.name,q:m.name,why:'Matched to '+pvTot+'W of panels on a 12V system, with Bluetooth built in.'});
          ...
          if(inv) items.push({cat:state.inv==='small'?'Inverter':'Inverter/charger',name:inv.name,q:inv.q,why:inv.why});
          ...
          root.querySelector('#vb-summary').innerHTML=
            ...
            '<span class="vb-sum-chip"><b>'+(inv?inv.name.replace('Victron ',''):'no inverter')+'</b></span>';
          root.querySelector('#vb-spec').innerHTML=items.map(function(it){
            return '<div class="vb-item"><div class="vb-item-cat">'+it.cat+'</div><div class="vb-item-name">'+it.name+'</div>'+
          ...
          root.__kit=items; root.__wiring=wiring;
        }
        var TOOLS_ESS=[
        ...
        root.querySelector('#vb-copy').addEventListener('click',function(){
          var btn=this, items=root.__kit||[], wiring=root.__wiring||[];
          ...
          var txt='My Victron build (via 365techies.co.uk/victron-system-builder/):\n\nKIT\n'+
            items.map(function(it){ return '• '+it.cat+': '+plain(it.name); }).join('\n')+
            '\n\nCABLES, LUGS & FUSES\n'+
            wiring.map(function(w){ return '• '+plain(w.cat)+': '+plain(w.name)+' — '+w.specs.map(plain).join(', '); }).join('\n')+
            '\n\nTOOLS: '+TOOLS_ESS.map(function(t){return plain(t[0]);}).join(', ');
